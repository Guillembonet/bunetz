...
func (*Static) Blog(c *gin.Context) {
	c.HTML(http.StatusOK, "", server.WithBase(c, blog.Blog()))
}

func (*Static) AboutWebsite(c *gin.Context) {
	c.HTML(http.StatusOK, "", server.WithBase(c, about_website.AboutWebsite()))
}
...
func (*Static) Echo(c *gin.Context) {
	...
	c.Negotiate(http.StatusOK, gin.Negotiate{
		...
		HTMLData: server.WithBase(c, echo.Echo(echoValue)),
